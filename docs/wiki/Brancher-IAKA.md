...
| `IAKA_JWT` | vide | Bearer d’exécution. Vide = pas d’appel (`IAKA_UNAVAILABLE`) |
| `IAKA_TENANT_ID` | vide | Tenant IAKA. Vide = `IAKA_UNAVAILABLE` |
...
Si l’URL, le JWT ou le tenant manque au moment d’un appel : code
`IAKA_UNAVAILABLE` (503), **aucun** fetch. Si IAKA répond mal : `IAKA_UPSTREAM`
(502). Voir [Dépannage](Depannage.md).
...
`GET /api/ready` reste `false`, **pas d’appel IAKA**, page « non configuré ».
Ne jamais coller d’UUID dans le wiki.
...
OpenAPI de chaque service : `server/*/openapi.json` dans le dépôt (et dans
l’image). Contrats fail-closed : sans Bearer, 401.
